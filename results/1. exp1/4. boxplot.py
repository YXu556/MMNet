...
titles = dict(zip(['corr', 'rmse', 'bias', 'ubrmse'], ['R', 'RMSE', 'Bias', 'ubRMSE']))
...
for model in models:
    for i, year in enumerate([2019, 2020, 2021, 2022]):
        if model == 'mmnet':
            ...
            output_dir = root / f"SCAN_USCRN_HLSL30_0.1_{model}_exp1_flatten_{'_'.join([str(year) for year in range(year - 3, year)])}"

        test_df = pd.read_csv(output_dir / f'y_test_pred_{year}.csv', index_col=0)
...
data = AllSamples[AllSamples.Year.isin([2019, 2020, 2021, 2022])].reset_index(drop=True)
model_metrics = defaultdict(list)
