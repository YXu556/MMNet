...
def load_result(model):
    model_dir = root / rf"SCAN_USCRN_HLSL30_0.1_{model}_exp3"
    for i, domain in enumerate(['W', 'M', 'E']):
        test_df = pd.read_csv(model_dir / domain / f'y_test_pred_ts.csv', index_col=0)
...
for year in [2019]:
    ...
    # print(data.columns)
    # for station_name, station in data.groupby('ID'):
    for station_name in stations:
        ...
        # lb = [in_situ] + pred1 + pred2 + pred3 + [pr]
