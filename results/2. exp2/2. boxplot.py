...
for model in models:
    if model == 'mmnet':
        output_dir = root / f"SCAN_USCRN_HLSL30_0.1_{model}_exp2"
    else:
        output_dir = root / f"SCAN_USCRN_HLSL30_0.1_{model}_exp2_flatten"
    for fold in range(1, 6):
        test_df = pd.read_csv(output_dir / f'Fold_{fold}' / f'y_test_pred.csv', index_col=0)
