"""
Script for training/evaluation of MMNet and Transformer
exp0&1&2
0: dependent (random spliting)
1: temporal transfer
2: spatial transfer
Also for eval of exp3, directly applying the coarse model on all fine-resolution data
"""
import copy
import json
import random
...
def parse_args():
    ...
    parser.add_argument('--lc_code', default='oh', help='how to encode LC, choose from [None, oh, bin]')
    ...
    parser.add_argument('-e', '--experiment', default=1, type=int,
                        help='no. of experiment setting choice from [0: dependent|1: on-site|2. off-site|3: cross-region]')
    ...
    parser.add_argument('--weights', type=str, default=None,
                        help='restore specific model to eval/finetune')
    ...
    parser.add_argument('--model', type=str, default="mmnet",
...
def main(args):
    ...
    if args.experiment == 2:
        ...
        print(f"Station results (median) : \n\n {scores_msg}")
...
def train_supervised(data, output_dir, args, train_idx=None, test_idx=None, return_pred=False):
    source_dataset, target_dataset = get_plain_dataset_ts(data, args.datapath, args, train_idx, test_idx)
    ...
    print("=> creating train/val/test dataloader")
    ...
    if not args.eval:
        ...
        for epoch in range(args.epochs):
            ...
            val_loss, scores = evaluation_ts(model, criterion, valdataloader, device)
            # lr_scheduler.step()
    ...
    if args.eval and args.weights is not None:
        best_model_path = Path(ft_weight_path)
    ...
    # test on valid
    if return_pred:
        val_loss, val_scores, y_val, y_pred_val = evaluation_ts(model, criterion, valdataloader, device, save_res=True)
        if args.save_pred:
            np.save(best_model_path.parent / 'y_val_pred.npy', np.hstack([y_val, y_pred_val]))
    else:
        val_loss, val_scores = evaluation_ts(model, criterion, valdataloader, device)
    ...
    if return_pred or args.save_pred:
        test_loss, scores, y_test, y_pred = evaluation_ts(model, criterion, testdataloader, device, save_res=True)
        if args.save_pred:
        ...
        test_loss, scores = evaluation_ts(model, criterion, testdataloader, device)
    ...
    elif args.eval and args.experiment == 3:
        fn = output_dir / "testlog_0.1.csv"
    else:
...
def train_epoch(model, optimizer, criterion, dataloader, device):
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader), leave=True) as iterator:
        for idx, (X_s, X_d, y) in iterator:
            ...
            # add l2 loss
            loss += sum(p.pow(2).sum() for p in model.parameters() if len(p.shape) == 2) / X_s.shape[
                0] * args.weight_decay
            loss /= 2
