...
class MMNet(nn.Module):
    def __init__(self, input_dim_s, input_dim_d, d_model=128, n_head=16, n_layers=1, d_inner=256,
        ...
        # statics
        encoder_s = [input_dim_s, 32, 64, d_model]
        layers = []
        for i in range(len(encoder_s) - 1):
            layers.extend([
                nn.Linear(encoder_s[i], encoder_s[i + 1]),
                nn.BatchNorm1d(encoder_s[i + 1]),
                nn.ReLU(),
                GaussianDropout(p=dropout)
            ])
        self.encoder_s = nn.Sequential(*layers)
    ...
    def forward(self, x_s, x_d, return_feats=False):
        ...
        # fusion
        f = self.fusion(torch.concat([f_s, f_d], dim=1))
